...
`enchant.stat_3948993189|33` 这种带管道符的 id，一开始我以为是「一个 stat 配一个
下拉框」，提取时全跳过了 —— 星团珠宝的小点词缀因此长期识别不了。**这个判断是错的。**

它们其实是 1798 条各自独立的词条，`text` 本身就是完整文本：

```
id   = enchant.stat_3948993189|33
text = Added Small Passive Skills grant: 12% increased Trap Damage
       Added Small Passive Skills grant: 12% increased Mine Damage
```

当普通词条收就行，界面上不需要任何额外交互。两个坑：

- **text 可能是多行的**（一条小点词缀同时给陷阱和地雷伤害，交易站记成一条 stat、
  两行文本，PoB 的物品文本里却是分开的两行）。每行都要当作这个 id 的别名收进对照表，
  两行才都认得出来；TSV 也塞不下换行。相应地，**同一个 id 在一次查询里只能出现一次** ——
  重复的筛选交易站会当成「要有两条这个词条」，直接搜不到东西。
- **同一效果有多个数值版本**（`10%` / `12% increased Attack Damage` 是不同的选项），
  归一化之后是同一个键。所以查表时**原文精确匹配必须排在归一化前面**，否则会挑错选项 ——
  而挑错的 id 在交易站上一样不报错。

真正需要下拉框的是带 `option` 字段的那 88 条，实测全是 `pseudo.*`，本来就在我们要的
命名空间之外。
