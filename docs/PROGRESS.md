...
2. **只有底子的图，传奇没有。** 传奇在游戏里有专属美术（`Shroud of the Lightless` 是
   `Art/2DItems/Armours/BodyArmours/AbyssBody`，GGG 的 CDN 实测 200），但 RePoE 的
   `base_items` 里没有传奇。查过的路都堵死了：RePoE 没有传奇数据文件、PoB 的
   `Data/Uniques/*.lua` 没有美术路径、poe.ninja 接口没查通、GGG 公共仓库流 403 要 OAuth、
   poedb 没有对外 JSON 接口。

   **poedb 之所以全都有，是因为它直接从游戏客户端的 `Content.ggpk` 里提取** ——
   那里面既有 `Data/*.dat` 数据表也有整棵 `Art/2DItems/**` 美术树，跑一次提取就全齐，
   版本更新重跑即可。这跟我们「消费别人整理好的 JSON」是两种数据管线。

   ### 补齐传奇图标的两条路

   **A. 爬 poedb 的分类列表页**（推荐先试）。已查证：`robots.txt` 是 `Allow: /`；
   `/us/Body_Armours` 这类列表页**一页就有 399 条**，HTML 里 `src` 是美术路径、
   `class="wN"` 是背包格宽、显示名在兄弟节点里。一个赛季几十个请求。
   **关键是我们只取映射表，图仍然从 GGG 的 CDN 取** —— 不消耗 poedb 的带宽，
   跟长期盗链人家 CDN 是两回事。待办：标记有变体（`alt` 有时是文件名有时是显示名），
   得用正经的 DOM 解析，正则硬凑试过两次都没配对上。

   **B. 自己从 `Content.ggpk` 提取**（poedb 的路子）。唯一能做到「永远最新、全都有」的路，
   但要本机装着游戏 + 一套提取工具，会把我们刚甩掉的「依赖本地安装」重新背回来，
   定时自动更新也就没了。
